...
- 앱 버전을 `0.7.3`으로 올리고, 실행 파일 메타데이터와 창 제목 표기를 함께 갱신했다.
- 그리드 원문/번역문 컬럼을 한 줄 미리보기로 고정하고, 긴 원문은 하단 선택 항목 보기에서 스크롤로 확인하도록 레이아웃을 안정화했다.
- 멀티라인 원문이 개행이나 공백으로 시작해도 그리드에서 빈 칸처럼 보이지 않도록 목록용 원문 미리보기에 `↵` 줄바꿈 표시를 추가했다.
- 원문 그리드 셀에 마우스를 올리면 실제 원문 전체를 툴팁으로 확인할 수 있게 했다.

### 수정

- 번역을 중지한 뒤 같은 실행에서 다시 시작할 때, 취소 시점의 현재 batch 항목이 `번역 중` 상태로 남아 재번역 대상에서 빠질 수 있던 문제를 수정했다.
- 취소된 현재 batch 항목은 즉시 `중지됨`으로 저장되도록 해 다시 시작 시 정상적으로 이어받게 했다.
- 빈 번역문이 `번역 완료`, `검수 필요`, `수동 수정` 같은 성공 계열 상태로 남는 경로를 상태 적용/수동 상태 변경에서도 차단했다.
- OpenAI API 설정이 Ollama/LAN Ollama 용도로 오인되지 않도록 `Ollama`를 명시적 별도 공급자로 유지하고, 기존 숫자 config가 공급자 enum 추가로 밀리지 않도록 숫자값을 고정했다.
...
- `Ollama` 공급자를 OpenAI API와 별도로 추가했다. 기본 URL은 `http://127.0.0.1:11434/v1`이며, 원격 PC의 Ollama OpenAI 호환 `/v1` 엔드포인트도 사용할 수 있다.
- Ollama 번역 경로는 JSON schema 응답을 먼저 실제 요청으로 검증하고, 서버나 모델이 JSON 구조를 지키지 못하면 tokenized fallback으로 재시도하도록 했다.
- Ollama도 로컬 모델 프리셋과 모델 목록 조회를 지원하며, 로컬/LAN 서버는 API Key 없이 사용할 수 있고 인증 프록시나 Ollama cloud를 쓸 때만 Bearer 키를 보낼 수 있게 했다.
...
- 빈 번역문이 `번역 완료`, `검수 필요`, `수동 수정` 같은 성공 계열 상태로 남지 않도록 상태 적용/수동 상태 변경 경로에서 `번역 실패 / 빈 번역문`으로 정규화했다.
- OpenAI API 공급자에 Ollama URL을 넣은 경우를 포트로 추정하던 임시 호환 처리를 제거하고, 명시적인 `Ollama` 공급자 선택으로 분리했다.
- 기존 숫자 enum config에서 공급자 값이 밀려 해석되지 않도록 `TranslationProviderType` 숫자값을 명시적으로 고정했다.
- Ollama OpenAI 호환 요청에는 공식 지원 필드 기준으로 `temperature`, `top_p`, `presence_penalty`, `seed`, `max_tokens`만 보내고 `top_k`/`repeat_penalty`는 제외하도록 했다.
- 번역 설정 창의 `LLM Glossary` 추가 옵션들이 겹쳐 보이던 레이아웃 문제를 수정했다.
- Windows x64 배포 zip에서 Linux/macOS/browser 및 x86/ARM runtime asset과 중복 publish 폴더를 제외하도록 정리했다.
